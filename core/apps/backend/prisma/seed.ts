...
async function main() {
  ...
  // Enable sales-order module (auto-enable for development)
  const salesOrderModule = await prisma.module.upsert({
    where: { moduleId: 'sales-order' },
    update: { enabled: true, installed: true },
    create: {
  ...
  console.log('✅ Enabled module:', salesOrderModule.moduleId)
