...
class ApiClient {
  ...
  async getEnabledModules(): Promise<any[]> {
    console.log('[API Client] 📡 Fetching all modules from /modules...')
    try {
      const response = await this.client.get<any[]>('/modules')
      console.log('[API Client] ✅ Received response:', {
        totalModules: response.data.length,
        modules: response.data.map((m: any) => ({ id: m.id || m.moduleId, name: m.name, enabled: m.enabled })),
      })
      const enabled = response.data.filter((module: any) => module.enabled)
      console.log(`[API Client] 🔍 Filtered ${enabled.length} enabled module(s) from ${response.data.length} total:`, enabled.map((m: any) => m.id || m.moduleId))
      return enabled
    } catch (error) {
      console.error('[API Client] ❌ Error fetching modules:', error)
  ...
  async getAllModules(): Promise<any[]> {
    ...
    try {
      const response = await this.client.get<any[]>('/modules')
      console.log('[API Client] ✅ Received all modules:', response.data.length)
      return response.data
    } catch (error) {
      console.error('[API Client] ❌ Error fetching all modules:', error)
      throw error
    }
  }
  ...
  async disableModule(moduleId: string): Promise<any> {
    const response = await this.client.post(`/modules/${moduleId}/disable`)
    return response.data
  }
